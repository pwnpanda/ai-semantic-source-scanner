...
def test_prep_on_tiny_ktor_indexes_kotlin_symbols(tmp_path: Path, fixtures_dir: Path) -> None:
    ...
    try:
        files = conn.execute("SELECT path FROM files WHERE path LIKE '%App.kt'").fetchall()
