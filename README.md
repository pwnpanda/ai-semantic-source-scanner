...
2. Reuse an existing `.venv` if present, or create one (`uv venv`), then `uv sync --all-groups`.
...
5. **Prompt for a container runtime:** `docker`, `podman`, or `none`. Accepts full word, single letter (`d`/`p`/`n`), or numeric (`1`/`2`/`3`).
6. **Prompt explicitly about installing Joern (~1.5 GB)** — defaults to "no". When chosen, downloads `joern-cli.zip` directly, validates the archive, extracts, and links binaries into `$HOME/.local/bin`.
7. Optionally copy bundled Claude Code skills into `~/.claude/skills/`.
...
**Environment variables** (override prompts and defaults; all optional):

| Variable | Values | Default | Effect |
|----------|--------|---------|--------|
| `AICS_NONINTERACTIVE` | `1` | unset | Never prompt; use defaults for every choice. |
| `AICS_RUNTIME` | `docker` \| `podman` \| `none` | prompts | Pick the container runtime without prompting. |
| `AICS_INSTALL_JOERN` | `yes` \| `no` | prompts | Force-install or skip Joern without prompting. |
| `AICS_REINSTALL_JOERN` | `yes` | unset | Reinstall Joern even if `joern` is already on `PATH`. |
| `AICS_JOERN_DIR` | path | `$HOME/.local/share/joern` | Directory the Joern distribution is extracted into. |
| `AICS_JOERN_LINK_DIR` | path | `$HOME/.local/bin` | Directory the `joern`, `joern-parse`, `*2cpg` binaries are symlinked into. |
| `AICS_JOERN_VERSION` | tag (e.g. `v2.0.350`) | latest release | Pin a specific Joern release instead of `latest`. |

Examples:

```bash
# Pick podman without prompting; install Joern into a custom location pinned to a version.
AICS_RUNTIME=podman AICS_INSTALL_JOERN=yes \
  AICS_JOERN_DIR=/opt/joern AICS_JOERN_VERSION=v2.0.350 \
  bash scripts/install.sh

# Force a clean Joern reinstall on the next run.
AICS_REINSTALL_JOERN=yes bash scripts/install.sh
```
